...
### QMT 双打与多账号轮动 (2026-06-13 记录)
- **底层测试结论**：成功联调国金 MiniQMT 和银河大 QMT。银河大 QMT 在挂载 v4.3.1 沙盘推演版 Socket Server 策略（解决并发与残影进程）后，可通过 `127.0.0.1:8888` 接收交易指令。
- **周末特性**：周末时 QMT 底层 C++ 行情接口 `get_full_tick` 和 `passorder` 会因停盘挂起并触发 Python 层的 10秒超时（此为正常预期）。**请务必于周一开盘后进行全链路实盘验证。**

### 券商优先级与通道策略 (IMPORTANT)
基于用户的费率折扣与开户情况，系统对下单通道执行以下严格优先级：
1. **银河 QMT（首选核心）**：系统主通道。
2. **通达信 / 华宝（特定狙击）**：专用于套利 162411 等华宝系基金，利用本家的赎回费折扣优势。
3. **国金 QMT（开发测试/社区版）**：仅用于原生 Socket 级别的纯净调试，或开源输出备用。

**【重要核心概念辨析：资金账号 vs 股东账号】**
- **深交所股东账号 (Stock Account)**：用户用于执行“T+7 星期轮动规避赎回费策略”的底牌，总共 6 个（3个深A+3个深TA基金户）。这是全国中登公司/深交所发放的**全网唯一编码**。这 6 个账号可以在银河、华宝、国金任何一个券商处同时挂载使用。
- **资金账号 (Funding Account / Client ID)**：券商内部发行的客户号（例如银河的 12 位纯数字账号），**不等于**股东账号。
- **🚨【QMT 系统级限制 (致命缺陷)】**：经过专家确认，量化终端 QMT **不支持**“一资多股”的内部路由。即使你在柜台绑定了 6 个股东账号，QMT 底层 `passorder` 也只认主股东号，无法通过传入股东账号字符串来实现分仓打新。此路已被证实走不通（由于用户拒绝再开 5 个独立资金账号）。

### 📅 【周一待办清单 (Monday Todo List)】
> **AI 助手请注意：周一开启新对话时，请直接阅读此清单并执行，无需重复询问用户背景。**
1. **测试 TDX 多账号轮动能力**：
   - 目标：测试通过 `TDX (通达信)` 接口（无论是银河版还是华宝版）能否接收并正确路由这 6 个不同的深交所股东账号（实现一拖六下单）。
   - 行动：要求用户在前端发单时选择【通达信(华宝)】，观察底层的 `trade_manager.py` 和 TDX 内存挂钩机制是否能成功切换股东账号完成报单。
2. **架构降级（如有必要）**：
   - 如果 TDX 测试成功：则将系统的发单通道（Order Routing）彻底锁定在通达信，而仅仅将银河 QMT 作为**纯粹的高速行情源**（只订阅，不下单），实现“QMT 看盘 + TDX 打单”的异步架构。
3. **完成实盘账本与赎回提醒功能**：
   - 在上述打单通道跑通后，继续完成程序 3 的后台账本系统（记录每天是用哪个尾号的股东账号买入的），并顺势写完 T+7 自动赎回提醒逻辑。
