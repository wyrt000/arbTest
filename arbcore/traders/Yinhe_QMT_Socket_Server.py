# encoding: gbk
# =================================================================
# v4.3.1 沙盘推演版 - 银河QMT Socket Server端策略 (更新时间: 2026-06-13)
# 优化：0. 修复 account_private 导入路径问题，支持真实账号读取。
#      1. 修复 __file__ 变量在 QMT 环境下不存在导致崩溃的 Bug。
...
# 隐私保护：不在此硬编码任何真实账号，避免提交到 Github 泄密。
# 实盘运行时将默认读取 QMT 界面右上角选中的资金账号。
QMT_ACCOUNT = ""  # 缺省为空
...
def process_command_sync(conn, cmd_str):
    ...
    if action == 'PING':
        ...
        # 支持动态指定账号：如果指令里带了第5个参数，则用指定的账号；否则用全局默认账号
        target_account = parts[4].strip() if len(parts) >= 5 else g_account_id
        
        opType = 23 if action == 'BUY' else 24
        if g_context and target_account:
            with g_api_lock:
                try:
                    msg = f"Socket_{action}_{code}"
                    # passorder 的第三个参数直接传入目标账号，完美支持多账号切换！
                    passorder(opType, 1101, target_account, code, 11, price, volume, 'SocketTrade', 1, msg, g_context)
...
def init(ContextInfo):
    ...
    print("\n[策略日志] 加载 v4.3.1 沙盘推演版 Socket 策略 (更新时间: 2026-06-13)...")
